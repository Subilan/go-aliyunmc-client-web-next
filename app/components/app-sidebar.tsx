...
const miscItems = [{ title: '更新日志', url: '/updates', icon: LogsIcon }];
			...
			<Sidebar collapsible="icon">
				<SidebarHeader>
					<div className="flex items-center gap-2 px-2 py-1">
						<span className="text-xl font-recursive-casual tracking-tight text-sidebar-foreground group-data-[collapsible=icon]:hidden">
							TiLab
						</span>
						<span className="hidden text-xl font-recursive-casual tracking-tight text-sidebar-foreground group-data-[collapsible=icon]:inline">
							Ti
						</span>
					</div>
					<SidebarMenu>
				...
				<SidebarContent>
						...
						<SidebarMenu>
							...
							{user?.whitelist_uuid && (
								<SidebarMenuItem>
									...
									>
										<Link
											to={`/game/statistics/${user.whitelist_uuid}`}
					...
					{(user?.role === 'operator' || user?.role === 'superuser') && (
						<SidebarGroup>
