...
function groupByMonth(items: ChangelogItem[]) {
  ...
  for (const item of items) {
    const key = dayjs(item.created_at).format('YYYY年M月');
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <div className="flex justify-center py-20">
          <Typography color="text.secondary">暂无更新日志</Typography>
        </div>
      ) : (
        <>
                    ...
                    >
                      <CardContent>
                        <div className="flex items-start justify-between gap-4 mb-2">
                          ...
                          <Chip
                            label={
                              update.category === 'platform' ? '控制台' : '服务器'
                            }
                            size="small"
                            variant="outlined"
                            color={
                              update.category === 'platform'
                                ? 'primary'
                                : 'success'
                            }
                            className="shrink-0"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          >
                            {dayjs(update.created_at).format('M月D日')}
                          </Typography>
                          <div className="flex items-center gap-0.5">
                            ...
                            >
                              <HeartIcon
                                size={16}
                                fill={update.liked ? 'currentColor' : 'none'}
                              />
                            </IconButton>
                            <Typography
                              variant="body2"
                              className="text-neutral-500 w-5 text-center"
                            >
                              {update.like_count}
                            </Typography>
                            {canEdit && (
        ...
        {selected && (
          <>
            <DialogTitle className="flex items-center justify-between gap-4">
              ...
              <Chip
                label={
                  selected.category === 'platform' ? '控制台' : '服务器'
                }
                size="small"
                variant="outlined"
                color={
                  selected.category === 'platform' ? 'primary' : 'success'
                }
                className="shrink-0"
              />
            </DialogTitle>
            <DialogContent dividers>
              <Typography variant="body2" className="text-neutral-500 mb-4">
                {dayjs(selected.created_at).format('YYYY年M月D日')}
              </Typography>
              {selected.body.split('\n').map((line, i) => (
                <Typography
                  key={i}
            ...
            <DialogActions className="justify-between px-6">
              <div className="flex items-center gap-0.5">
                <IconButton
                  ...
                  className={
                    selected.liked ? 'text-red-500' : 'text-neutral-400'
                  }
                >
                  <HeartIcon
                    size={16}
