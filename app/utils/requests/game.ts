import { get, type Resp } from '~/utils/requests';
...
export async function getSortedAdvancements(uuid: string) {
	const res = await getAdvancements(uuid);
	if (res.error || !res.data) {
		return res;
	}
	res.data.sort((a, b) => a.englishName.localeCompare(b.englishName));
	return res;
}
